...
# get out j_ssn3 data and adjust with jitter to dist_nearest_up_wb_km
# this effectively creates a giant spread so that there's almost 0 observed effect for the change of dist_nearest_up_wb_km when wbc is absent
jitter_obs = ssn_get_data(j_ssn3)
jitter_obs$dist_nearest_up_wb_km[jitter_obs$wbc == "absent"] =  runif(nrow(jitter_obs[jitter_obs$wbc == "absent",]), -100000000000, 100000000000)
# put back into an ssn object
jit_ssn = SSN2::ssn_put_data(jitter_obs,j_ssn3)


a5ssn_wswq_ml1 <- ssn_lm(
  formula = vsci ~ pct_imp_w_emplog + elev_ws + do + vahusb_JL + wbc + wbc:dist_nearest_up_wb_km,
  ssn.object = jit_ssn,
  tailup_type = "none",
  taildown_type = "exponential",
  euclid_type = "exponential",
...
## alternate option for just numerical (This matches roughly what they did in the paper you sent)
num_obs = ssn_get_data(j_ssn3)
str(num_obs$wbc)
num_obs$wbc = as.character(num_obs$wbc)
num_obs$wbc[num_obs$wbc == "absent"] = 0
num_obs$wbc[num_obs$wbc == "present"] = 1
num_obs$wbc = as.numeric(num_obs$wbc)
num_ssn = SSN2::ssn_put_data(num_obs,j_ssn3)
...
#### mike's model questions

####### through to the end of this chunk is just the models we talked about on the call
# wbc only
mod1 <- ssn_lm(
formula = vsci ~ pct_imp_w_emplog + elev_ws + do + vahusb_JL + rck_dep_ws + wwtp + wbc,
ssn.object = j_ssn3,
...
# DistNearestUpWb_km only

mod2 <- ssn_lm(
formula = vsci ~ pct_imp_w_emplog + elev_ws + do + vahusb_JL + rck_dep_ws + wwtp + dist_nearest_up_wb_km,
ssn.object = j_ssn3,
tailup_type = "none",
taildown_type = "exponential",
euclid_type = "exponential",
...
# wbc and DistNearestUpWb_km

mod3 <- ssn_lm(
formula = vsci ~ pct_imp_w_emplog + elev_ws + do + vahusb_JL + rck_dep_ws + wwtp + wbc + dist_nearest_up_wb_km,
ssn.object = num_ssn,
tailup_type = "none",
taildown_type = "exponential",
euclid_type = "exponential",
...
mod4 = ssn_lm(
formula = vsci ~ pct_imp_w_emplog + elev_ws + do + vahusb_JL + rck_dep_ws + wwtp +wbc+ wbc:dist_nearest_up_wb_km,
ssn.object = num_ssn,
tailup_type = "none",
taildown_type = "exponential",
euclid_type = "exponential",
